...
const ViewCard = () => {
  ...
  const total = cart?.lineItems?.reduce(
    (acc: number, item: any) => acc + item.price.amount * item.quantity,
    0
  );
        ...
        <div className="grid gap-6">
          ...
          {cart.lineItems?.map((item: any) => (
            ...
            >
              ...
              {/* PRICE */}
              <div className="font-bold text-xl text-lama dark:text-lama-green">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-lg mt-6 flex flex-col items-center gap-4">
            <div className="text-2xl font-bold dark:text-gray-100">
              Total:{" "}
              <span className="text-lama dark:text-lama-green">
                ${(total ?? 0).toFixed(2)}
              </span>
            </div>
            <div className="">
